...
/** Level4Plus.java
  ...
  * description: finding Duber's path in order to maximize tips
  */
class Level4Plus{
  ...
  public static int bestTip;
  public static boolean tipFound = false;
  ...
  public static void main(String[] args)throws Exception{
    ...
    rows = readFile.nextInt(); // Find how many rows and columns are in Duber's Map
    ...
    System.out.print("The maximum tip is: ");
    System.out.println(bestTip);
    System.out.print("Amount of steps: ");
    System.out.println(totalSteps);
    for(int i=0; i<rows; i++){ //print solution map
      for(int j=0; j<columns; j++){
  ...
  public static int[][] charToInt(char[][] map){
    ...
    for(int i=0; i<rows; i++){ //changes characters into representative integers
      for(int j=0; j<columns; j++){
        if(map[i][j] == '#'){
          intMap[i][j] = -1;          
        }else if(map[i][j] == ' '){
  ...
  public static char[][] findBest(int[][] map,int position, int[][] destinations, int[] path, char[][] readMap, char[][] bestMap){ 
    ...
    if(position == destCount - 1){//possible path is found
      ...
      steps = map[destinations[path[0]][0]][destinations[path[0]][1]];
      time += steps;
      pathMap = addPath(map, destinations[path[0]][0], destinations[path[0]][1], steps, pathMap); //add shortest path to map
      pathMap[destinations[path[0]][0]][destinations[path[0]][1]] = 'X';
      stepTip = (readMap[destinations[path[0]][0]][destinations[path[00]][1]] - '0') - time; // Calculate tip based on delivery number and time
      if (stepTip >0){
        time = stepTip * 10;
      }else{
         tip = stepTip;
      ...
      for(int i=0; i<destCount - 1; i++){
        ...
        map = findSteps(map, destinations[path[i]][0], destinations[path[i]][1] ); //find distance to everything else
        steps = map[destinations[path[i+1]][0]][destinations[path[i+1]][1]]; //find how many steps to the next destinatin
        time += steps;
        stepTip = (readMap[destinations[path[i+1]][0]][destinations[path[i+1]][1]] - '0') - time; // Calculate tip
        if (stepTip > 0){
          tip += stepTip * 10;
        }else{
          tip += stepTip; 
        ...
        pathMap[destinations[path[i+1]][0]][destinations[path[i+1]][1]] = 'X'; // mark the destination
      }
      if((tip > bestTip) || (!tipFound)){
        bestTip = tip;
        tipFound = true;
        totalSteps = time;
